...
async def main():
    ...
    start_tconst = "tt0073195"  # Specify the starting tconst value
    end_tconst = "tt0100156"    # Specify the ending tconst value
        ...
        async with session.get(url) as response:
            if response.status == 200:
                with gzip.open(BytesIO(await response.read()), 'rt', encoding='utf-8') as file:
                    ...
                    while True:
                      if last_inserted_tconst and last_inserted_tconst <= end_tconst:
                        movie_df = df[(df['tconst'] > last_inserted_tconst) & (df['titleType'] == 'movie') & (df['originalTitle'].notna()) & (df['tconst'] <= end_tconst)].head(batch_size)
                      else:
                        movie_df = df[(df['tconst'] >= start_tconst) & (df['titleType'] == 'movie') & (df['originalTitle'].notna()) & (df['tconst'] <= end_tconst)].head(batch_size)
                        ...
                        last_inserted_tconst = movie_df['tconst'].max()
